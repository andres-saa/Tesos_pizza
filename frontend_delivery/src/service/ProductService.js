...
import { useSitesStore } from "../store/site";
 ...
 const productService = {
    ...
    async getProductsByCategorySite(category_id,site_id,restaurant_id = 5) {
        const store = useSitesStore()
        const restaurant = store.restaurant_id
        try {
            const response = await axios.get(`${URI}/products-all/category-id/${category_id}/site/${site_id}/${5}`);
