...
{{
  ...
  formatoPesosColombianos(
    (product.is_combined ? flavor.price / 2 : flavor.price) * product.quantity
        ...
        <!-- Botones para seguir comprando o finalizar pedido... -->
        <router-link to="/" v-if="route.path.includes('cart')">
          <Button
            ...
            class="mt-4"
            severity="danger"
            style="outline: none; width: 100%; font-weight: bold;"
          />
        </router-link>
        ...
        <!-- Si el local está cerrado, advertir. De lo contrario, permitir finalizar pedido -->
        <Tag
          v-if="siteStore.status.status == 'closed'"
          severity="danger"
          style="width: 100%; text-transform: capitalize; height: 2.5rem;"
          class="mt-2"
        >
          Cerrado, abrimos a las {{ siteStore.status.next_opening_time.split(':')?.slice(0,2).join(' : ') }}
        </Tag>
        ...
        <router-link to="/pay" v-if="route.path.includes('cart') && siteStore.status.status != 'closed'">
          <Button
            ...
            class="mt-2"
            severity="help"
            style="outline: none; width: 100%; border: none; font-weight: bold; background-color: black;"
          />
        </router-link>
  
        <router-link v-else-if="siteStore.status.status != 'closed'" to="/pay">
          <Button
            :disabled="store.sending_order || siteStore.status.status == 'closed'"
            @click="() => {
  ...
  // Aquí agrupamos las adiciones globales (no asociadas a un producto específico).
  const agrupados = ref({});
  ...
  onMounted(() => {
    ...
    // Si no se ha establecido un precio de entrega
  ...
  /* Ajusta estilos según tu preferencia */
  </style>
