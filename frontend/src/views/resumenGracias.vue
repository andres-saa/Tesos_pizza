    ...
    <div class="p-0 my-1 md:my col-12" v-if="store.cart?.products?.length > 0">
      ...
      >
        ...
        >
          ...
          <!-- Sabores -->
          <div
            style="display: flex; gap: 0.3rem; align-items: center; justify-content: start; flex-wrap: wrap;"
          >
            <!-- Recorremos cada sabor -->
            <div
              v-for="(flavor, index) in product.flavors"
              :key="index"
              style="display: flex; width: 100%; gap: 0.2rem; align-items: center; justify-content: space-between;"
            >
              <Tag style="padding: .3rem; text-transform: capitalize;">
                ...
                <span v-if="product?.quantity > 1 && flavor.price > 0">
                  x {{ product.quantity }}
                </span>
              </Tag>
